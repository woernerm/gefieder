...
class AccessConfig(AuthConfig):
    ...
    meaning much the same. Only the display name changes; the app label stays "auth".
    """
...
class SsoConfig(AppConfig):
    ...
    def ready(self):
        ...
        # post_migrate fires once per installed app, so the receiver is bound to this
        # one. Listed last, so the permissions the groups hand out already exist.
        post_migrate.connect(create_role_groups, sender=self)

        # By the time this signal fires the session is the one the person keeps, so a
        # session Django rotated on the way in cannot swallow the URL.
