/*
 * echarts-simple-transform 1.0.0 (Apache-2.0), vendored from npm because ECharts ships
 * no aggregate transform of its own -- core has filter, sort and boxplot only, and the
 * handbook's "Aggregate" example is mis-filed under ecStat, which does not provide one
 * either. Kept unminified so the modification below is readable.
 *
 * MODIFIED from the released 1.0.0: the SUM creator returned a literal 0, so the first
 * row of every group was discarded (a group of 1,2,3 summed to 5). It now seeds the
 * accumulator with the first row's value, which is what the AVERAGE creator alongside it
 * already did. Every other method released upstream computes correctly; see
 * analytics/tests.py, which asserts the sums this file is relied on for.
 *
 * Upstream is dormant (last release 2022-04) and describes itself as non-official, so
 * this stays a vendored copy rather than a dependency.
 *
 * The sourceMappingURL comment the release ends with is also removed: the map is
 * not shipped, and whitenoise's manifest storage refuses to collect a reference it
 * cannot resolve.
 */
