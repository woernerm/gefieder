...
# The address Grafana believes it is reached at, which it puts into every absolute link
# it builds -- and its %(protocol)s is what Grafana serves inside the pod, http, the proxy
# terminating TLS. A production system is reached over https, so those links would name
# the wrong scheme. Settled here from DEBUG, a quadlet not being able to branch on a
# runtime setting.
